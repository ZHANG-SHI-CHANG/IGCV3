...
class IGCV3FPN():
    ...
    def __build(self):
        ...
        with tf.variable_scope('zsc_feature'):
            ...
            x = IGCV3Block('IGCV3_1',x,self.BlockInfo['1'][0],self.BlockInfo['1'][1],self.BlockInfo['1'][2],self.BlockInfo['1'][3],self.BlockInfo['1'][4],
                                       self.BlockInfo['1'][5],self.norm,self.activate,self.is_training)
            skip_1 = x
            #none,none/2,none/2,16
            
            x = IGCV3Block('IGCV3_2',x,self.BlockInfo['2'][0],self.BlockInfo['2'][1],self.BlockInfo['2'][2],self.BlockInfo['2'][3],self.BlockInfo['2'][4],
                                       self.BlockInfo['2'][5],self.norm,self.activate,self.is_training)
            skip_2 = x
            #none,none/4,none/4,24
            
            x = IGCV3Block('IGCV3_3',x,self.BlockInfo['3'][0],self.BlockInfo['3'][1],self.BlockInfo['3'][2],self.BlockInfo['3'][3],self.BlockInfo['3'][4],
                                       self.BlockInfo['3'][5],self.norm,self.activate,self.is_training)
            skip_3 = x
            #none,none/8,none/8,32
            
            x = IGCV3Block('IGCV3_4',x,self.BlockInfo['4'][0],self.BlockInfo['4'][1],self.BlockInfo['4'][2],self.BlockInfo['4'][3],self.BlockInfo['4'][4],
                                       self.BlockInfo['4'][5],self.norm,self.activate,self.is_training)
            skip_4 = x
            #none,none/16,none/16,64
            
            x = IGCV3Block('IGCV3_5',x,self.BlockInfo['5'][0],self.BlockInfo['5'][1],self.BlockInfo['5'][2],self.BlockInfo['5'][3],self.BlockInfo['5'][4],
                                       self.BlockInfo['5'][5],self.norm,self.activate,self.is_training)
            skip_5 = x
            #none,none/16,none/16,96
            
            x = IGCV3Block('IGCV3_6',x,self.BlockInfo['6'][0],self.BlockInfo['6'][1],self.BlockInfo['6'][2],self.BlockInfo['6'][3],self.BlockInfo['6'][4],
                                       self.BlockInfo['6'][5],self.norm,self.activate,self.is_training)
            skip_6 = x
            #none,none/32,none/32,160
            
            x = IGCV3Block('IGCV3_7',x,self.BlockInfo['7'][0],self.BlockInfo['7'][1],self.BlockInfo['7'][2],self.BlockInfo['7'][3],self.BlockInfo['7'][4],
                                       self.BlockInfo['7'][5],self.norm,self.activate,self.is_training)
            skip_7 = x
            #none,none/32,none/32,320
            
            x = IGCV3Block('IGCV3_8',x,self.BlockInfo['8'][0],self.BlockInfo['8'][1],self.BlockInfo['8'][2],self.BlockInfo['8'][3],self.BlockInfo['8'][4],
        ...
        if Detection_or_Classifier=='classifier':
            ...
            with tf.variable_scope('zsc_detection'):
                with tf.variable_scope('zsc_conv_transpose'):
                    skip = tf.concat([skip_7,skip_6],axis=-1)
                    x = _conv_block('Conv_0',skip,skip.get_shape().as_list()[-1],1,1,'SAME',self.norm,self.activate,self.is_training)
                    pred1_x = x
                    x = tf.image.resize_bilinear(x,[2*tf.shape(x)[1],2*tf.shape(x)[2]])
                    
                    skip = tf.concat([skip_5,skip_4],axis=-1)
                    ...
                    x = _conv_block('Conv_3',x,x.get_shape().as_list()[-1],1,1,'SAME',self.norm,self.activate,self.is_training)
                    pred3_x = x
                    
                with tf.variable_scope('zsc_pred'):
                    self.pred_yolo_1 = _conv_block('Conv_0',pred1_x,self.num_anchors*(5+self.num_classes),1,1,'SAME',self.norm,self.activate,self.is_training)
                    #none,none/32,none/32,5*(5+self.num_classes)
                    self.pred_yolo_2 = _conv_block('Conv_1',pred2_x,self.num_anchors*(5+self.num_classes),1,1,'SAME',self.norm,self.activate,self.is_training)
                    #none,none/16,none/16,5*(5+self.num_classes)
                    self.pred_yolo_3 = _conv_block('Conv_2',pred3_x,self.num_anchors*(5+self.num_classes),1,1,'SAME',self.norm,self.activate,self.is_training)
...
################################################################################################################
##IGCV3Block
def IGCV3Block(name,x,ratio=6,num_filters=32,repeat=1,stride=1,group=2,use_depthwise=True,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        for i in range(repeat):
            input = x
            
            x = _group_conv('group_conv_{}_0'.format(i),x,group,ratio*num_filters,1,1,'SAME',norm,activate,is_training)
            if use_depthwise:
                if stride==2:
                    if i==0:
                        x = _depthwise_conv2d('depthwise_{}_0'.format(i),x,1,3,2,'SAME',norm,activate,is_training)
                    else:
            ...
            if stride==1:
                if input.get_shape().as_list()[-1]==x.get_shape().as_list()[-1]:
                    ...
                    input = _conv_block('conv_{}_2'.format(i),input,num_filters,1,1,'SAME',norm,activate,is_training)
                x += input
            else:
                pass
        return x
...
def PrimaryConv(name,x,num_filters=32,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        #none,none,none,3
        x = _conv_block('conv',x,num_filters,3,2,'SAME',norm,activate,is_training)#none,none/2,none/2,num_filters
...
if __name__=='__main__':
    ...
    model = IGCV3FPN(num_classes=len(labels),
                      num_anchors=5,
                      batch_size = batch_size,
                      max_box_per_image = max_box_per_image,
